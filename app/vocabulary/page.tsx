        ...
        {showUnitPicker && (
          <div className="absolute z-40 mt-1 w-full sm:w-80 bg-white border rounded-lg shadow-lg py-1 max-h-96 overflow-y-auto">
            <button
              ...
              className={`w-full text-left px-4 py-2.5 text-sm hover:bg-neutral-50 transition-colors ${filterUnit === "all" ? "bg-lavender-100 font-medium" : ""}`}
            >
              全部單元（{items.length} 字）
            </button>
            {units.map((unit) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-4 py-2.5 text-sm hover:bg-neutral-50 transition-colors ${filterUnit === unitKey ? "bg-lavender-100 font-medium" : ""}`}
                >
                  <div className="flex items-center justify-between">
